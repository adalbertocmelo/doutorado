...
#include "ns3/tcp-multi-stream-helper.h"
...
*/
void
getThropughputClients(ApplicationContainer clientApps, TcpMultiStreamClientHelper clientHelper, std::vector <std::pair <Ptr<Node>, std::string> > clients)
{
...
static void
ServerHandover(ApplicationContainer clientApps, TcpMultiStreamClientHelper clientHelper, Address server2Address, std::vector <std::pair <Ptr<Node>, std::string> > clients, uint16_t n)
{
  clientHelper.Handover(clientApps, clients.at (n).first, server2Address);
}

void
getStartTime(ApplicationContainer clientApps, TcpMultiStreamClientHelper clientHelper, std::vector <std::pair <Ptr<Node>, std::string> > clients)
{
  double STsv1=0;
  double STsv2=0;
  double STsv3=0;
...
void
stopSim (TcpMultiStreamClientHelper clientHelper, NodeContainer staContainer)
{
...
void
getStall(ApplicationContainer clientApps, TcpMultiStreamClientHelper clientHelper, std::vector <std::pair <Ptr<Node>, std::string> > clients)
{
  double sv1=0;
  double sv2=0;
  double sv3=0;
...
void
getClientsStallsRebuffers(ApplicationContainer clientApps, TcpMultiStreamClientHelper clientHelper, std::vector <std::pair <Ptr<Node>, std::string> > clients)
{
  for (uint i = 0; i < numberOfClients; i++)
  {
...
void
getClientsHandover(ApplicationContainer clientApps, TcpMultiStreamClientHelper clientHelper, std::vector <std::pair <Ptr<Node>, std::string> > clients)
{
  queue[0]=0;
  queue[1]=0;
  queue[2]=0;
...
uint64_t
getRepIndex(ApplicationContainer clientApps, TcpMultiStreamClientHelper clientHelper, Ptr <Node> clients)
{
  return clientHelper.GetRepIndex(clientApps,clients);
}
/*
void
politica(ApplicationContainer clientApps, TcpMultiStreamClientHelper clientHelper, std::vector <std::pair <Ptr<Node>, std::string> > clients,ApplicationContainer serverApp, TcpStreamServerHelper serverHelper,NodeContainer servers)
{
  ...
  for (uint i = 0; i < numberOfClients; i++)
  {
    ...
    {
      std::string ip = clientHelper.GetServerAddress(clientApps, clients.at (i).first);
      std::string filename = "python3 src/Fog4VR/AHP/AHP.py " + dirTmp +" "+ToString(simulationId)+" "+delays[0]+" "+delays[1]+" "+delays[2]+" "+delays[3]+" "+ip+" "+ToString(Tf1)+" "+ToString(Tf2)+" "+ToString(Tf3)+" "+ToString(Tf4);
...
void
politica2(ApplicationContainer clientApps, TcpMultiStreamClientHelper clientHelper, std::vector <std::pair <Ptr<Node>, std::string> > clients,ApplicationContainer serverApp, TcpStreamServerHelper serverHelper,NodeContainer servers)
{
  getClientsOnServer(serverApp, serverHelper, servers);
  getClientsHandover(clientApps,clientHelper,clients);
  getClientsStallsRebuffers(clientApps,clientHelper,clients);
...
void ThroughputMonitor(FlowMonitorHelper *fmhelper, Ptr<FlowMonitor> flowMon)
{
  ...
  for (auto stats : flowStats)
  {
    ...
    if (Simulator::Now() - stats.second.timeLastTxPacket > Seconds(1) &&
		Simulator::Now() - stats.second.timeLastRxPacket > Seconds(1))
    {
      continue;
    }
  ...
  Simulator::Schedule(Seconds(1), ThroughputMonitor, fmhelper,
                      flowMon);
}
...
SPAddress = fogAddress[7];
  ...
  std::vector<Ipv4Address> aux_addresses(fogAddress);
  //remove cloud address to prevent duplication
  auto toRemove = std::find(aux_addresses.begin(), aux_addresses.end(), SPAddress);
  aux_addresses.erase(toRemove);

  TcpMultiStreamClientHelper clientHelper (SPAddress, port,pol);
  ...
  clientHelper.SetAttribute ("AuxiliaryAddresses", AttributeContainerValue <AddressValue> (aux_addresses));
...
/*
  //TcpMultiStreamClientHelper clientHelper2 (server2Address, port);
  ...
  //TcpMultiStreamClientHelper clientHelper3 (server3Address, port);
...
/*
  ...
  /* Install TCP/UDP Transmitter on the station */
  //TcpMultiStreamClientHelper clientHelper (serverAddress, port); //NS_LOG_UNCOND("dash Install 288");
  //clientHelper.SetAttribute ("SegmentDuration", UintegerValue (segmentDuration));
  //clientHelper.SetAttribute ("SegmentSizeFilePath", StringValue (segmentSizeFilePath));
  //clientHelper.SetAttribute ("NumberOfClients", UintegerValue(numberOfClients));
...
/*
  ...
  TcpMultiStreamClientHelper clientHelper2 (serverAddress2, port);
  ...
  Ptr<FlowMonitor> flowMonitor;
	FlowMonitorHelper flowHelper;
	flowMonitor = flowHelper.Install(networkNodes);
