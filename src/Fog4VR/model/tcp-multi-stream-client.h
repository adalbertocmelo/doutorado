...
#ifndef TCP_MULTI_STREAM_CLIENT_H
#define TCP_MULTI_STREAM_CLIENT_H
 ...
 */
class TcpMultiStreamClient : public Application
{
...
public:
  ...
  TcpMultiStreamClient ();
  virtual ~TcpMultiStreamClient ();
...
private:
  ...
  template <typename T>
  void
  SendAuxiliary (T & message, uint16_t index);

  void
  SendMultiple (uint32_t numBytes, std::vector<int> serverIds=std::vector<int>());
   ...
   */
  void LogDownload (Address server, uint64_t socket_bytes);
  /*
   ...
   *
   * The output streams defined in TcpMultiStreamClient are opened,
  ...
  std::vector<Ptr<Socket>> m_auxiliary_sockets;
  Address m_peerAddress; //!< Remote peer address
  std::vector<Address> m_auxiliary_peers;
  Address newip;
  Address serverIpv4;
  ...
  std::map<Address, int64_t> m_subsegment_size; //!< Stores the sizes of partial segments
  std::map<Address, int64_t> m_socketBytesReceived; //!< Counts the amount of received bytes of the current socket
...
#endif /* TCP_MULTI_STREAM_CLIENT_H */
